...
describe("Router contract", ()=> {
    ...
    describe("Deposit liquidity", () => {
        async function deployRouterFixture() {
            ...
            /* AAVE/DAI, 1 AAVE = $310, 1 DAI = $1 USD */
            ...
            const amountBMin = ethers.utils.parseUnits('307', 18); //DAI
        ...
        describe("should only allow a deposit of two ERC20 tokens of equal value", () => {
            ...
            it("should deposit amountADesired and amountBDesired for a new liquidity pool", async() => {
                ...
                const formattedAmountAReturned = ethers.utils.formatUnits(amountA);
                const formattedAmountBReturned = ethers.utils.formatUnits(amountB);

                /* ASSERT */
                expect(formattedAmountAReturned).to.equal(formattedAmountADesired);
                expect(formattedAmountBReturned).to.equal(formattedAmountBDesired);
            });
            ...
            it("should deposit the optimal ratio of tokens for an existing pool", async() => {
               ...
               /* ACT */
                const { amountA, amountB } = await router.callStatic.depositLiquidity(
                    aaveToken.address,
                    daiToken.address,
                    ethers.utils.parseUnits('56', 18), //AAVE simulate error deposit
                    ethers.utils.parseUnits('310', 18), //DAI simulate error deposit
                    amountAMin,
                    amountBMin,
                    liquidityProvider.address,
                    deadline
        ...
        describe("should mint the correct number of Liquidity Tokens", () => {
            ...
            it("for an existing liquidity pool", async() => {
                ...
                const totalSupply = ethers.utils.parseUnits("17.606816861659009145");
                ...
                const formattedLiquidityTokensMinted = ethers.utils.formatUnits(liquidity);

                /* ASSERT */
                expect(formattedLiquidityTokensMinted).to.equal(formattedMininum);
            });
